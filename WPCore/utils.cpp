...
namespace utils
{
    std::wstring multiByte2WideChar(const char *str, bool from_utf8 /* = true */) {
        if (str == nullptr) {
            Logger::instance().error(L"multiByte2WideChar: str is nullptr.");
            return std::wstring();
        }
        ...
        if (len <= 0) {
            Logger::instance().error(L"multiByte2WideChar failed.");
            return std::wstring();
        }
    ...
    std::string wideChar2MultiByte(const wchar_t *str, bool to_utf8 /* = true */) {
        if (str == nullptr) {
            Logger::instance().error(L"wideChar2MultiByte: str is nullptr.");
            return std::string();
        }
        ...
        if (len <= 0) {
            Logger::instance().error(L"wideChar2MultiByte failed.");
            return std::string();
        }
    ...
    int internetGet(const std::string &host, const std::string &path, std::string &content,
        ...
        if (res) {
            ...
            Logger::instance().error(
                std::format(L"ResponseError: {}", multiByte2WideChar(httplib::to_string(res.error()).c_str()))
            );
            return 0;
        }
    }
